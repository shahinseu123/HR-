             ...
             <div class="card-body">
                 <div class="row">
                    <div class="col-md-6">
                        ...
                        <select name="vendor" v-model="form.vendor" id="customer" class="form-control" :class="{ 'is-invalid': form.errors.has('vendor') }">
                            <option value="">Select vendor</option>
                            <option v-for="v in vendors.data" :key="v.id" :value="v.name">{{v.name}}</option>
               ...
               <div class="row">
                   <div class="col-md-6">
                       ...
                       <div class="form-group"> 
                            <input v-model="form.purchasedate" type="date" name="purchasedate" placeholder="Enter Purchase Order date"
               ...
               <div class="row">
                   <div class="col-md-6">
                      ...
                      <div class="form-group"> 
                            <input v-model="form.purchasenumber" type="number" name="purchasenumber" placeholder="Enter Purchase Order number"
                                class="form-control" :class="{ 'is-invalid': form.errors.has('purchasenumber') }">
                            <has-error :form="form" field="purchasenumber"></has-error>
                    </div>
                   </div>
                    ...
                    <tbody>
                          <tr id="item-row-0" v-for="(item, index) in rowData" :key="item.index">
                            ...
                            <!-- hidden  -->
                                ...
                                <input v-model="$data[latestPurchase.id ? item.purchase_id = latestPurchase.id : '0']" class="form-control text-right"  name="price" type="hidden" id="item-price-0">
...
export default {
    ...
    methods: {
        ...
        submitPurchase(){
            ...
            setTimeout(() =>  {this.getLatestPurchase()}, 1000)
            // setTimeout(() =>  {this.getLatestInvoice()}, 3000);

            setTimeout(() => {
                 this.rowData.forEach((item) => { 
                 axios.post('api/purchase-product', item)
                    .then(() => {
                        console.log('success')
        ...
        getLatestPurchase(){
            axios.get('api/purchase-product')
                 .then(({data}) => {
                      this.latestPurchase = data
                 })
        }
